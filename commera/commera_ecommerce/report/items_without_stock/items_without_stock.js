...
frappe.query_reports['Items Without Stock'] = {
	...
	onload: (report) => {
		report.page.add_inner_button(__('Unpublish'), async () => {
			...
			await frappe.call({
				method:
					'commera.commera_ecommerce.report.items_without_stock.items_without_stock.bulk_unpublish',
				args: { filters },
			});
		...
		report.page.add_inner_button(__('Publish'), async () => {
			...
			await frappe.call({
				method:
					'commera.commera_ecommerce.report.items_without_stock.items_without_stock.bulk_publish',
				args: { filters },
			});
