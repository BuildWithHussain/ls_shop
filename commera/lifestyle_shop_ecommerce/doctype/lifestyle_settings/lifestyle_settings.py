...
class LifestyleSettings(Document):
	...
	if TYPE_CHECKING:
		...
		from commera.lifestyle_shop_ecommerce.doctype.footer_section_mapping.footer_section_mapping import (
			FooterSectionMapping,
		)
		from commera.lifestyle_shop_ecommerce.doctype.item_group_map.item_group_map import ItemGroupMap
		from commera.lifestyle_shop_ecommerce.doctype.return_reason.return_reason import ReturnReason
		from commera.lifestyle_shop_ecommerce.doctype.search_content_field.search_content_field import (
			SearchContentField,
		)
		from commera.lifestyle_shop_ecommerce.doctype.search_result_field.search_result_field import (
			SearchResultField,
		)
	...
	def enqueue_publish_all_variants(self, attribute: str):
		...
		frappe.enqueue(
			"commera.lifestyle_shop_ecommerce.doctype.lifestyle_settings.lifestyle_settings.generate_configurators_for_all_templates",
			queue="long",
